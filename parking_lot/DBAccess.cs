...
namespace parking_lot
{
    class DBAccess
    {
        ...
        private static string strConnString = "Data Source=(local);Initial Catalog=parking_lot;Integrated Security=True";
        ...
        public int executeQueyWithInsertedId ( SqlCommand dbCommand )
        {
            try
            {
                if ( connection.State == 0 )
                ...
                dbCommand.CommandText = dbCommand.CommandText + " ; SELECT SCOPE_IDENTITY()";
                dbCommand.Connection = connection;
                dbCommand.CommandType = CommandType.Text;

                int primaryKey = Convert.ToInt32(dbCommand.ExecuteScalar());
                return primaryKey;
            }
            catch ( Exception ex )
            {
